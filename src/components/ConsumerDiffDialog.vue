    ...
    <div v-loading="loading" class="diff-container">
      <div v-if="!loading && diffData" class="diff-content">
        <div class="diff-header">
          <div class="diff-info">
            <el-tag type="info" size="small">Consumer: {{ consumerName }}</el-tag>
...
import { getConsumerDiff } from '@/api/consumers'
...
const fetchDiff = async () => {
  ...
  try {
    const response = await getConsumerDiff(props.consumerId)
    diffData.value = response
  } catch (err: any) {
    console.error('获取Consumer diff失败:', err)
